 ...
 *     description: Returns list of manufacturers with device counts
 *     tags: [Database]
 */
router.get('/manufacturers', async (req, res) => {
  try {
    ...
    const formattedStats = manufacturers
      .map(row => ({ 
        name: row.manufacturer, 
        count: row.device_count 
      }))
      .sort((a, b) => b.count - a.count)
      .slice(0, 50); // Top 50 manufacturers
    
    res.json(formattedStats);
  } catch (error) {
    console.error('Database manufacturers error:', error);
    res.status(500).json({ 
      error: error.message,
      timestamp: new Date().toISOString()
    });
 ...
 *   get:
 *     summary: Get device type statistics
 *     description: Returns list of device types with counts
 *     tags: [Database]
 */
router.get('/device-types', async (req, res) => {
  try {
    ...
    // Transform the data to match frontend expectations  
    const formattedStats = deviceTypes.map(row => ({
    ...
    console.error('Database device-types error:', error);
    res.status(500).json({ 
      error: error.message,
      timestamp: new Date().toISOString()
    });
 ...
 *   get:
 *     summary: Get recent database updates
 *     description: Returns list of recent database updates and changes
 *     tags: [Database]
 */
router.get('/updates', async (req, res) => {
  try {
    ...
    console.error('Database updates error:', error);
    res.status(500).json({ 
      error: error.message,
      timestamp: new Date().toISOString()
    });
 ...
 *     description: Returns coverage statistics and data quality metrics
 *     tags: [Database]
 */
